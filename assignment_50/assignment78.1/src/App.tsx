...
function App() {
  ...
  const handleUncheck = (taskIndex: number) => {
  ...
  return (
    <div className="flex flex-col h-screen text-black gap-5 p-4 bg-gray-100">
      ...
      <div className="flex justify-between items-center ">
        ...
        <button
          className="bg-yellow-400 hover:bg-yellow-500 hover:shadow-2xs hover:shadow-black text-white font-semibold px-3  py-3 rounded-lg shadow-md transform transition-transform duration-300 hover:scale-102"
      ...
      <div className="flex flex-col gap-2 space-evenly items-start ml-2 shadow-lg ">
        ...
        <div className="ml-4 mb-4 ">
          <div
            className={`overflow-hidden transition-all duration-500 ease-in-out ${formshow ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
              }`}
          >
            {formshow && (
              <ToDoForm onSave={handleAddTask} onCancel={handleCancel} />
            )}
          </div>

          {buttonShow && (
            <button
              className="bg-yellow-400 hover:bg-yellow-500 hover:shadow-2xs hover:shadow-black text-white font-semibold px-3  py-3 rounded-3xl shadow-md transform transition-transform duration-300 hover:scale-102"
              onClick={handleAdd}
            >
              + Add To-Do
            </button>
